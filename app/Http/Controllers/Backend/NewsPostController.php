...
class NewsPostController extends Controller
{
    ...
    public function GetSubCategoryPosts($id) {
        $subposts = NewsPost::with('user', 'subcategory')->withCount('comments')->where('subcategory_id', $id)->latest()->limit(4)->get();
        return json_encode($subposts);
    }

    public function GetCategoryPosts($id) {
        if (Subcategory::where('category_id', $id)->first()) {
            $subposts = NewsPost::with('user', 'category', 'subcategory')->withCount('comments')->where('category_id', $id)->latest()->limit(4)->get();
        } else {
            $subposts = NewsPost::with('user', 'category')->withCount('comments')->where('category_id', $id)->latest()->limit(4)->get();
        }
        return json_encode($subposts);
    }
}
